...
public class Birthdays {
	...
	public static void main(String[] args) {
		...
		// first response
		System.out.println("Today is " + month + "/" + day + "/2024, day #" + today + " of the year.");
		System.out.println();

		// ask birthday questions
		int firstDay = askBirthday(scanner, "Please enter person #1's birthday (month day): ", today);
		int secondDay = askBirthday(scanner, "Please enter person #2's birthday (month day): ", today);

		// output logic
		if (firstDay == secondDay)
			System.out.println("Wow, you share the same birthday! ^_^");
		else if (today - firstDay > today - secondDay)
	...
	public static int askBirthday(Scanner scanner, String question, int today) {
		...
		int length;
		if (absoluteDay >= today)
			length = absoluteDay - today;
		else
			length = 366 - (absoluteDay - today);
		...
		// print out output
		System.out.print(month + "/" + day + "/2024 falls on day #" + absoluteDay + " of 366. ");

		// logic for extra output
		if (length == 0)
			System.out.println("Happy Birthday! :)");
		else
			System.out.println("Your next birthday is in " + length + " day(s).");
