...
// Code provided by project instructor, only changed thing is a newly added ``nextLetter()`` method and a private array list.
...
public class WordList extends ArrayList<String> {
    ...
    try {
      ...
      try {
        while (scanner.hasNext()) {
          ...
          for (int i = 0; i < word.length(); i++) array.add(
            word.charAt(i) + ""
          );
  ...
  public void add(int index, String s) {
    if (Collections.binarySearch(this, s) != -index) super.add(-index, s);
    else {
      throw new RuntimeException(
        "Adding " + s + " at position " + index + " would break the word order"
      );
    }
  }
   ...
   * nextLetter method
   *
   * @return the next most probable letter that will appear based off of the word
   *         list using an array that contains all letters
   */
