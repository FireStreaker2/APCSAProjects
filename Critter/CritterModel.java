...
public class CritterModel {
  ...
  public void add(int number, Class critter) {
    if (myList.size() + number > myWidth * myHeight) throw new RuntimeException(
      "adding too many critters"
    );
    for (int i = 0; i < number; i++) {
      ...
      try {
        ...
        throw new RuntimeException(
          "no zero-argument constructor for " + critter
        );
      }
      if (!(instance instanceof Critter)) {
        throw new RuntimeException(critter + " does not implement Critter");
      }
    ...
    if (!critterCount.containsKey(name)) critterCount.put(name, number);
    else critterCount.put(name, critterCount.get(name) + number);
  }
  ...
  private void updateDisplay() {
    // set it to all dots
    for (int x = 0; x < myWidth; x++) for (int y = 0; y < myHeight; y++) if (
      myGrid[x][y] == null
    ) display[x][y] = '.';
    else display[x][y] = myGrid[x][y].getChar();
  }
  ...
  private void movePoint(Point p, int direction) {
    if (direction == Critter.NORTH) p.y = (p.y + myHeight - 1) % myHeight;
    else if (direction == Critter.SOUTH) p.y = (p.y + 1) % myHeight;
    else if (direction == Critter.EAST) p.x = (p.x + 1) % myWidth;
    else if (direction == Critter.WEST) p.x = (p.x + myWidth - 1) % myWidth;
    else if (direction != Critter.CENTER) throw new RuntimeException(
      "Illegal direction"
    );
  }
