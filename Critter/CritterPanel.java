...
public class CritterPanel extends JPanel {
	...
	public CritterPanel(CritterModel model) {
		...
		setPreferredSize(new Dimension(FONT_SIZE * model.getWidth() / 2 + 125, FONT_SIZE * model.getHeight() + 20));
	}
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < width; i++)
			for (int j = 0; j < height; j++)
				g.drawString("" + myModel.getChar(i, j), extraX / 2 + i * charWidth, extraY / 2 + j * FONT_SIZE);
	}
}
