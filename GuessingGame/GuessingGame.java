...
public class GuessingGame {
  ...
  public static void main(String[] args) {
    ...
    while (input.charAt(0) == 'y') {
      ...
      if (currentGuesses < bestGuesses || games == 1) bestGuesses =
        currentGuesses;
  ...
  public static int startGame(Scanner scanner, Random random) {
    ...
    // end immediately)
    while (answer == guess) answer = random.nextInt(max - min + 1) + min;
    ...
    while (guess != answer) {
      ...
      if (scanner.hasNextInt()) {
        ...
        if (guess > max || guess < min) System.out.println(
        ...
        if (
          attempts != 1 &&
          (Math.abs(answer - guess) > Math.abs(answer - previousGuess))
