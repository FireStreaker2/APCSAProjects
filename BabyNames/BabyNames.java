...
public class BabyNames {
	 ...
	 */
	public static void start(Scanner userInput, Scanner fileInput) throws FileNotFoundException {
		// ask for user input
		...
		while (fileInput.hasNextLine() && running) {
			...
			while (lineInput.hasNext()) {
				if (lineInput.next().equalsIgnoreCase(name))
					running = false;
			}
		}
		...
		if (running)
			...
			PrintStream output = new PrintStream(
					new File(name.toUpperCase().charAt(0) + name.toLowerCase().substring(1, name.length()) + ".txt"));
