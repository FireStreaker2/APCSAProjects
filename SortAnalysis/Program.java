...
public class Program {
	public static void main(String[] args) {
		...
		for (int i = 0; i < firstTests.length; i++) {
			...
			for (int j = 0; j < firstTests[i].length; j++)
				firstTests[i][j] = random.nextInt(Integer.MAX_VALUE);
		}
		...
		for (int i = 0; i < secondTests[0].length; i++)
			secondTests[0][i] = i;

		for (int i = secondTests[1].length - 1; i >= 0; i--)
